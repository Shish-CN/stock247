...
export function transformCandlePoints(payload, maximumPoints = 36) {
  if (!Array.isArray(payload)) throw new Error("invalid_candle_payload");
  const points = payload
    .map((candle) => ({
      time: finiteNumber(candle?.t),
      close: finiteNumber(candle?.c)
    }))
    .filter((point) => point.time !== null && point.close !== null)
...
function apiBase() {
  return (process.env.HYPERLIQUID_API_BASE || DEFAULT_API_BASE).replace(/\/$/, "");
}

export async function fetchTradeXyzMarkets() {
  const payload = await fetchJson(`${apiBase()}/info`, {
    method: "POST",
...
export async function fetchTradeXyzCandles(symbol, options = {}) {
  const normalized = normalizeSymbol(symbol);
  if (!normalized) throw new Error("invalid_symbol");
  const endTime = Number.isFinite(options.endTime) ? options.endTime : Date.now();
  const startTime = Number.isFinite(options.startTime) ? options.startTime : endTime - 24 * 60 * 60 * 1000;
  const interval = options.interval || "1h";
  const payload = await fetchJson(`${apiBase()}/info`, {
    method: "POST",
    body: JSON.stringify({
      type: "candleSnapshot",
      req: { coin: `xyz:${normalized}`, interval, startTime, endTime }
    })
  });
  return transformCandlePoints(payload);
}

export function parseLimit(value, fallback = 20, maximum = 500) {
